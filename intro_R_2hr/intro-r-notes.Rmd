...
After your project is created, navigate to its directory using your Finder/File explorer. You will see the `.RProj` file has been created. Next, move or save the [`data.csv`](data.csv) file from the workshop materials into the project directory so that we can use it in this workshop.
...
Since this is something you would need to repeat in future, it should go in your `main.R` script.
...
dat <- read.table(file="data/data.csv", sep=",", header=TRUE, stringsAsFactors=FALSE)
```
...
1. To prepare for future workshops, install the following packages on your machine.
    - `tidyverse`
    - `lubridate`
    - `cowplot`
