...
# Data description
The data in this workshop were collected as part of an on-going oceanographic time series program in Saanich Inlet, a seasonally anoxic fjord on the East coast of Vancouver Island, British Columbia (Figure 1A). The data we will use include various geochemical measurements at many depths in Saanich Inlet. Samples were taken approximately monthly from 2006 to 2014, though there is much missing data to contend with.

<br>
<br>
(A) ![](images/Saanich.png){width=4in} (B) ![](images/Inlet_structure.png){width=3in}

**Figure 1.** (A) Map of Saanich Inlet indicating conventional sample collection stations (S1-S9). Data used in this workshop is sourced from S3. (B) Structure of Saanich Inlet. The glacial sill restricts water circulation into and out of the lower depth of the inlet basin.
<br>
<br>

Saanich Inlet is a steep sided fjord characterized by a shallow glacial sill located at the mouth of the inlet that restricts circulation in basin waters below 100 m (Figure 1B). During spring and summer months, elevated primary production (like photosynthesis) in surface waters combined with restricted circulation results in progressive water column stratification and complete oxygen starvation (anoxia) in deep basin waters. In late summer, pulses of oxygenated nutrient-rich ocean waters up-welling from the Haro Straight cascade over the sill, displacing oxygen starved bottom waters upward. The intensity of these renewal events varies from year to year with implications for microbial ecology and biogeochemical cycles (Figure 3). 
...
Let's create a project to work in during this workshop.

In the top-right corner of your RStudio window, click the "Project: (None)" button to show the projects drop-down menu. Select "New Project..." > "New Directory" > "New Project." Under directory name, input "Intermediate_R" and choose a parent directory to contain this project on your computer.

Then copy or move the provided [data files](https://github.com/EDUCE-UBC/workshop_data) into your project directory.

## Installing and loading packages
For your convenience, we have provided `main.R`, which contains all of the code you will need to run for this workshop as well as spaces for you to complete exercises. Please work from the `main.R` document and use these notes for reference.
...
In this workshop, we will use the [tidyverse](https://www.tidyverse.org/) and lmerTest for data analysis and function building. We will also use devtools and roxygen2 for package building. Make sure you've downloaded these packages before attempting to load them.
```{r, message=FALSE}
library(tidyverse)
library(lmerTest)
library(devtools)
...
Let's start by reading in the data we will use for this workshop. Here, we specify that the data file is named "Saanich_Data.csv". Furthermore, we specify that the first row of the data contain variable names, and that empty, NA, NAN, or ND values should be treated by R as missing data (coded in R as NA). 
...
Then, we will clean the data using functions in the tidyverse.
```{r}
dat <- 
  raw_data %>%
...
*If you would like to learn more about functions in the tidyverse, please see our R tidyverse workshop.*
...
![](images/RStudio.png){width=0.4in} Exercise. 
...
Similarly, we can pull out pieces of statistical model calculated in R since the output is a list object. For example, we previously calculated a linear model of oxygen by depth with `m1 <- lm(O2 ~ Depth, data=dat)`. If we wanted to see only the coefficients of this model, we would use
...
![](images/RStudio.png){width=0.4in} Exercise. For the following, try to determine what the function will return *without running the code* then check yourself by running it.
...
2. Calculate a linear model for 1 variable by depth (Let's do oxygen)
...
It looks like our output is a vector of length 2 with names. If we try to simply write the `pval` object to the hard drive, we receive an error because it is a vector, not a data frame.
```{r error=TRUE}
write_csv(pval, path="pval_results.csv")
```

We can get around this by forcing the output into a data frame.
```{r}
...
But oh no! We've successfully saved our p-values but without any labeling! Fortunately, the loop in our function can save us! But first, let's add `write_csv` to our function to see how it works with the loop. Remember to update your comments!
...
Since `read_csv` overwrites any previous versions of the table, we end up with just the p-values from the last variable listed in `y`. You can, instead, save every iteration of the loop with `append=TRUE`.
...
But now we see that we've double-saved values for the last variable in `y` (first 2 and last 2). This occurred because `append` added to the file that was already there from when we ran `lm.function` without the `append` parameter. This becomes even more clear if you repeatedly run the function.
...
We remedy this by removing any previous results files before we run our loop again.
```{r message=FALSE}
lm.function <- function(data, cruise, x, y){
  # Load necessary packages
  require(tidyverse)
...
Now we have all the results in table, but a list of p-values on its own isn't very useful. So, let's reformat these data. The first thing to consider is what you want the output to look like. Our goal will be:
...
1. Transpose the `pval` output so it's 1x2 instead of 2x1
2. Add names for each iteration of the loop
3. Add column names

#### Step 6.2.1: Transpose
Transposing the output is simply done with `t()`.
```{r message=FALSE}
lm.function <- function(data, cruise, x, y){
  # Load necessary packages
  require(tidyverse)
...
Since we want the loops to be named based on their corresponding `y.variable`, we can add a column for this value. This means that we'll need to make `pval` into a data.frame a little earlier as well.
```{r message=FALSE}
lm.function <- function(data, cruise, x, y){
  # Load necessary packages
  require(tidyverse)
...
If we try to add column names in our `write_csv` function, the names are added for every iteration of the loop, so we actually need to add these data outside of the loop function (*i.e.* after the loop's ending `}`).

There are a number of ways to do this. My preferred method is to let the loop write the results to a table as is and then outside the loop, read in that table, add column names, and re-save it, thus overwriting the previous, unlabeled version.
```{r message=FALSE}
lm.function <- function(data, cruise, x, y){
  # Load necessary packages
  require(tidyverse)
...
Let's go one step further and make all of the names dynamic, *i.e.* based on the function's inputs.

Note how we can create a dynamic name using data from the `pval` output (example, col1) or from the `function` inputs (examples, col2 and table.name). Since these names are created within the local function environment, we call them in `rename` using the syntax `!!as.name(col1) := X1`.
```{r message=FALSE}
lm.function <- function(data, cruise, x, y){
  # Load necessary packages
  require(tidyverse)
...
The final step in any analysis, but especially in functions, is to go over your documentation and commenting to make sure everything is clear. Best practices are to include an introduction to the function that includes the purpose, license, and input parameters.
...
Finally, you should share your function with the world! Consider posting on GitHub or another open source site. You'd be surprised how many people might find your function useful as-is or as a starting off point to customize for their own needs.
...
![](images/RStudio.png){width=0.4in} CHALLENGE. You may not have time in this workshop to complete these challenging exercises. However, we encourage you to complete them on your own to test your knowledge of functions and improve your coding skills!
...
#'
#' Calculates p-values from linear models of geochemical data from a single Saanich Inlet cruise
#'
...
dat <- read_csv(file="../data/Saanich_Data.csv",
                     col_names=TRUE,
                     na=c("", "NA", "NAN", "ND")) %>% 
  select(Cruise, Date, Depth, 
         Temperature, Salinity, Density, 
...
Non-standard files/directories found at top level:
  ‘Depth_lm_pvals.csv’ ‘pval_results.csv’
...
lm.function: no visible global function definition for ‘write_csv’
lm.function: no visible global function definition for ‘read_csv’
lm.function: no visible global function definition for ‘rename’
lm.function: no visible global function definition for ‘:=’
lm.function: no visible binding for global variable ‘X1’
lm.function: no visible binding for global variable ‘X2’
lm.function: no visible binding for global variable ‘X3’
Undefined global functions or variables:
  %>% := Cruise X1 X2 X3 lm read_csv rename write_csv
Consider adding
  importFrom("stats", "lm")
...
(In reality, you would not want to publicly share a function that prints to the hard-drive since everyone's directory structure is unique. This is the cause of most of the notes above. You would instead print the results to the R environment and then the user could choose to save to a file.)
