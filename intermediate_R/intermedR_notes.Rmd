...
Please come to the workshop with your laptop setup with the required software and data files as described in our [setup instructions](setup/intermedR_setup.html).
...
library(lmerTest)
# Tools for package creation and development
library(devtools)
library(roxygen2)
```
...
```{r}
raw_data <- read_csv(file="data/Saanich_Data_clean.csv",
                     col_names=TRUE,
                     na=c("", "NA", "NAN", "ND"))
```

*If you would like to learn more about how these data were cleaned and manipulated, please see our [R tidyverse workshop](https://github.com/EDUCE-UBC/workshops_R/tree/master/intro_tidyverse).*
...
#### Exercise: Scoping
![](images/RStudio_logo.png){width=0.4in} Exercise. For the following, try to determine what the function will return *without running the code* then check yourself by running it.
...
#### CHALLENGE Exercise: Altering dynamic functions
