...
```{r chick weight plot, echo = FALSE, fig.height = 4}
chick_plot <- ggplot(chickwts, aes(x = feed, y = weight, color = feed)) +
  ...
  xlab("Type of diet") +
  ylab("Weight gain over 6 wks (grams)") +
  ylim(0, 450)
chick_plot
```
...
**Null Hypothesis, $H_{0}$**: $\mu_{caesin} = \mu_{horsebean} = \mu_{linseed} = \mu_{meatmeal} = \mu_{soybean} = \mu_{sunflower}$  
...
Let's now load the fruit fly data from the `faraway` package, and create our categorical groups from the numerical variables. *If you are unfamiliar with the functions below, checkout our [R tidyverse workshop](https://github.com/EDUCE-UBC/workshops_R/tree/master/intro_tidyverse).*
...
#### Exercise: ANOVA
![](images/RStudio_logo.png){width=0.4in} Exercise. Determine whether the following statements are *true or false*?
...
![](images/RStudio_logo.png){width=0.4in} Exercise. At your table, discuss the following questions.
...
### Tranforming predictors: polynomial
...
1. Check which _covariates_ have a significant effect on the response in the model fitted with the Poisson family and with the quasi-Poisson family and
