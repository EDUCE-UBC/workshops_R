...
1. `gtr -cat 20`: the model for tree calculation
    * Checkout the help page for other models
2. `-constraints`: topological constraints (the fasta file we just made)
3. `-nt`: a nucleotide alignment (the modified alignment we just made)
4. `>`: a name for the output tree
...
We can then do the same for phylum instead of domain. 
...
Compare the two trees. What do you observe? Which do you think better represents these data?

# Re-root the tree
We hope that you come to the same conclusion that the constrained tree is a better fit because known taxonomic groups (domains and phyla) cluster more together closely in this tree. We can further improve the constrained tree by adding a root.
...
**We will use the constrained, rooted tree in all subsequent analyses.**
...
and then run the analysis for Jaccard using our custom functions. This can take several minutes to complete.
...
Next, run the same analysis for Bray-Curtis.
